...
export class InstagramDownloader {
    ...
    try {
      ...
      const videoData = await page.evaluate(() => {
        const title = document.querySelector('meta[property="og:title"]')?.getAttribute('content') || 
                      document.querySelector('title')?.textContent || 'Instagram Video';
      ...
      throw new Error(`Failed to get Instagram video info: ${(error as Error).message}`);
    } finally {
      if (browser) {
        await browser.close();
      }
  ...
  async download(url: string, quality: string): Promise<string> {
    ...
    try {
      ...
      const outputPath = `/tmp/instagram_${quality === 'audio' ? 'audio' : 'video'}_${timestamp}.${extension}`;
      ...
      throw new Error(`Failed to download Instagram video: ${(error as Error).message}`);
    } finally {
      if (browser) {
        await browser.close();
      }
